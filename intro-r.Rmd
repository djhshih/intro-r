...
Can you recite the keys on a standard keyboard in order from top to bottom and left to right?

\includegraphics{img/keyboard.pdf}

\bigskip

Yet your fingers usually type the write key.
...
- Routinely tested across platform:  
...
Elements of a list can be different types (**heterogeneous container**)
...
`data.frame` a special type of `list` where all the fields have the same length.
...
This behaviour is achieved by the **copy-on-write mechanism**
...
Therefore, to update an object, the function must return the modified object,
...
Final exam grades for both groups are availabe in the `data/control.vtr`
and `data/treatment.vtr`.
...
g1 <- 100 - rpois(20, 25);
g2 <- 100 - rpois(21, 20);
write.table(g1, "data/control.vtr", row.names=FALSE, col.names=FALSE);
write.table(g2, "data/treatment.vtr", row.names=FALSE, col.names=FALSE);
```

```{r, echo=FALSE, eval=FALSE}
# read input data

g1 <- read.table("data/control.vtr", header=FALSE)[,1];
g2 <- read.table("data/treatment.vtr", header=FALSE)[,1];
...
In programming languages, punctuations and formatting similar serve to improve 
clarity and reduce ambiguity in programming languages. However, some programming
languages rely more on punctuations, while others rely more on formatting.
